"""Scenario where a request is rejected before the lane later becomes free.

ID 1 first has to slow down behind the decelerating vehicle ahead. After the
adjacent lane becomes safe without cooperation, it performs the lane change.
"""
...
def build_obstacles():
    """Build the longer two-lane road used by the rejected-request scenario."""
    road_end_x = 125
    ox, oy = [], []
...
def main(*, show_animation: bool = True, save_output_dir: str | None = None, save_animation: bool = False):
    """Set up and run the reject-then-free-lane scenario."""
    print("start simulation: rejected_request_then_free_lane")

    ox, oy, ox2, oy2, obstacle_list = build_obstacles()
    vehicle_configs = build_default_rejected_request_then_free_lane_configs()
    ...
    print("end simulation: rejected_request_then_free_lane")
    ...
    if save_output_dir:
        ...
        plt.close()
        plt.subplots(1)
        plot_road_boundaries(main_color="k", middle_color="y", road_end_x=125.0, lane_lines=(0.0, 4.0, 8.0))
