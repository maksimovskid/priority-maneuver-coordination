"""Runnable two-vehicle coordination scenario.

The requester reacts to a slower lead vehicle and negotiates with one target
lane vehicle that can adapt its trajectory to open a safe lane-change gap.
"""
...
def build_obstacles():
    """Build a two-lane road and static boundary obstacles for planning."""
    ox, oy = [], []
    for i in range(0, 80):
        ox.append(i)
        oy.append(0.0)
    ...
    obstacle_list = [(i, 0) for i in range(0, 71)] + [(i, 8) for i in range(0, 85)]
...
def main(*, show_animation: bool = True, save_output_dir: str | None = None, save_animation: bool = False):
    """Set up and run the basic request/accept two-vehicle simulation."""
    print("start simulation: two_vehicles_coordination")

    ox, oy, ox2, oy2, obstacle_list = build_obstacles()
    vehicle_configs = build_default_two_vehicle_coordination_configs()
    ...
    result = run_two_vehicle_coordination_scenario(
    ...
    print("end simulation: two_vehicles_coordination")
    ...
    if save_output_dir:
        ...
        plt.close()
        plt.subplots(1)
        plot_road_boundaries(main_color="k", middle_color="y", road_end_x=80.0, lane_lines=(0.0, 4.0, 8.0))
