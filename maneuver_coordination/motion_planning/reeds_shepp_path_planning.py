...
class Path:
    """Container for one candidate Reeds-Shepp curve sequence."""
...
def mod2pi(x):
    """Normalize an angle to the Reeds-Shepp [-pi, pi] interval."""
...
def straight_left_straight(x, y, phi):
    """Solve one straight-left-straight Reeds-Shepp primitive."""
    phi = mod2pi(phi)
...
def set_path(paths, lengths, ctypes):
    """Append a unique feasible primitive sequence to the candidate list."""
    path = Path()
    path.ctypes = ctypes
...
def straight_curve_straight(x, y, phi, paths):
    """Generate straight-curve-straight candidate families."""
...
def left_straight_left(x, y, phi):
    """Solve one left-straight-left primitive."""
...
def left_right_left(x, y, phi):
    """Solve one left-right-left primitive."""
...
def curve_curve_curve(x, y, phi, paths):
    """Generate curve-curve-curve candidate families."""
...
def curve_straight_curve(x, y, phi, paths):
    """Generate curve-straight-curve candidate families."""
...
def left_straight_right(x, y, phi):
    """Solve one left-straight-right primitive."""
    u1, t1 = polar(x + math.sin(phi), y - 1.0 - math.cos(phi))
    u1 = u1 ** 2
    if u1 >= 4.0:
        u = math.sqrt(u1 - 4.0)
...
def generate_path(q0, q1, max_curvature):
    """Generate all Reeds-Shepp candidate paths between two poses."""
    dx = q1[0] - q0[0]
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2]
    c = math.cos(q0[2])
...
def interpolate(ind, length, mode, max_curvature, origin_x, origin_y, origin_yaw, path_x, path_y, path_yaw, directions):
    """Interpolate one sampled point along a primitive segment."""
    if mode == "S":
...
def generate_local_course(total_length, lengths, mode, max_curvature, step_size):
    """Sample a local Reeds-Shepp course from segment lengths and types."""
...
def calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size):
    """Generate and transform candidate paths into the global frame."""
    q0 = [sx, sy, syaw]
    q1 = [gx, gy, gyaw]
...
def reeds_shepp_path_planning(sx, sy, syaw,
    ...
    """Return the shortest Reeds-Shepp path between start and goal poses."""
...
def main():
    """Run the standalone Reeds-Shepp demo."""
